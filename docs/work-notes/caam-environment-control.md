...
- Coherent checkpoints now cover the foundation (`be99bc4b5`), bounded client (`f0b2d6aa9`), snapshot-validation hardening (`24c641591` and `d3c0206a9`), formatting reconciliation (`802cde60c`), settings UI (`5bf27d79b`), verification reconciliation (`662333949`), plan-bound fallback restoration (`987aef824`), fork/PR closure (`ab13abd58`, `9e668f35f`, and `00bc13e2b`), CI compiler remediation (`d9db8915f`), and locale completion (`765f3dd82`).
...
- Synthetic Swift tests exist for configuration, contract validation, row projection, exact command construction, failure redaction, settings persistence, and coordinator refresh. They have not executed on this VPS, but the repository CI executed and passed the complete Linux test suite on x64 and ARM64 at implementation head `d9db8915fa23beead7d2beb58fb4472b5d0b7a55`.
- Repository CI also passed SwiftLint, release builds and CLI smoke tests on Linux x64 and ARM64, and the static Linux-musl build at implementation head `d9db8915fa23beead7d2beb58fb4472b5d0b7a55`. The pull request's native aggregate gate requires macOS tests after the pull request leaves draft state.
- Required PR CI supplies the macOS compile, app-locale, and synthetic-test gate at the exact merge candidate. UI screenshot, live CAAM, SSH gateway, credential, and mutation evidence remain outside this foundation slice.
